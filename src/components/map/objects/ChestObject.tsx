...
import { Box, Torus, Float, Sparkles } from '@react-three/drei';
...
export const ChestObject = memo(function ChestObject({ obj }: { obj: MapObject }) {
  return (
    <group position={[obj.x, obj.y, obj.z]}>
      ...
      <Box args={[0.85, 0.15, 0.65]} position={[0, 0.55, 0]} castShadow>
      ...
      <Torus args={[0.06, 0.02, 6, 8]} position={[0, 0.35, 0.31]} rotation={[Math.PI / 2, 0, 0]}>
        <meshStandardMaterial color="#FFD700" emissive="#FFD700" emissiveIntensity={1} metalness={0.9} roughness={0.15} />
      </Torus>
      <Float speed={2} rotationIntensity={1} floatIntensity={0.3} floatingRange={[0.4, 0.6]}>
      ...
      <Sparkles count={8} scale={1.2} size={2} speed={0.3} color="#FFD700" opacity={0.5} />
